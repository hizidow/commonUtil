...
for (i in cost)
{y1=substr(cmd_begin_time[i],0,4);m1=substr(cmd_begin_time[i],6,2);d1=substr(cmd_begin_time[i],9,2);
...
printf("%.2f %.2f\t",cmd_num[i]/(mktime(y2" "m2" "d2" "h2" "M2" "s2)-mktime(y1" "m1" "d1" "h1" "M1" "s1)),cost[i]/cmd_num[i]);
printf("%s %s\t",cmd_begin_time[i],cmd_end_time[i]);
printf "\n"}
}' input.txt
